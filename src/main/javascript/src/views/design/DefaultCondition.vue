	...
	<div>
		...
		>
			...
			<fieldset class="rounded-lg p-5 mb-7" v-if="experiment.conditions">
				<label  v-for="condition in experiment.conditions" :key="condition.conditionId" :for="`condition-${condition.conditionId}`">
				<span>{{ condition.name }}</span>
					<span class="radio-check" >
						<input name="selectedDefault" v-model="selectedDefault" :value="condition.conditionId" :id="`condition-${condition.conditionId}`" type="radio" required />
		...
		<v-card
			...
			outlined
		>
			<p><strong>Note:</strong> It's important to specify a default condition so that we know what version of assignments students should receive if they're not participating in the experiment.  This condition should be similar to business-as-usual; the sort of assignment that students would complete during the normal conduct of the course.</p>
		</v-card>
	</div>
</template>
			...
			this.setDefaultCondition({conditions, defaultConditionId})
					.then(response => {
						if (response?.status === 200) {
							this.$router.push({name:'ExperimentDesignSummary', params:{experiment: this.experiment.experimentId}})
						}
					})
					.catch(response => {
