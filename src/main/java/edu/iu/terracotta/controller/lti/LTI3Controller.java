    ...
    public String home(HttpServletRequest req, Principal principal, Model model) throws DataServiceException, CanvasApiException, ConnectionException {
        ...
        try {
            ...
            if (BooleanUtils.isTrue(ltiDataService.getDemoMode())) {
                return handleDemoMode(model, link, lti3Request);
            }
        ...
        } catch (SecurityException | GeneralSecurityException | IOException | URISyntaxException | AssignmentNotCreatedException ex) {
            model.addAttribute(TextConstants.ERROR, ex.getMessage());
            return TextConstants.LTI3ERROR;
        }
    }

    private String handleDemoMode(Model model, String link, LTI3Request lti3Request) {
        ...
        if (StringUtils.isNotBlank(link)) {
            ...
            log.debug("Searching for link {} in the context Key {} and id {}", link, lti3Request.getContext().getContextKey(), lti3Request.getContext().getContextId());
        ...
        if (lti3Request.getLtiMessageType().equals(LtiStrings.LTI_MESSAGE_TYPE_DEEP_LINKING)) {
            ...
            if (CollectionUtils.isEmpty(ltiLinkRepository.findByLinkKeyAndContext(ltiLinkEntity.getLinkKey(), ltiLinkEntity.getContext()))) {
            ...
            if (CollectionUtils.isEmpty(ltiLinkRepository.findByLinkKeyAndContext(ltiLinkEntity2.getLinkKey(), ltiLinkEntity2.getContext()))) {
